...
public class MainActivity extends AppCompatActivity {
    ...
    private WechatCloudManager wechatCloudManager;  // 微信云开发管理器
    ...
    private static final long PREVIEW_INTERVAL = 2000;  // 2秒上传一帧预览
    ...
    private void handleRemoteCommandFromIntent(Intent intent) {
        ...
        // 检查微信小程序命令
        String wechatAction = intent.getStringExtra("wechat_action");
        String wechatCommandId = intent.getStringExtra("wechat_command_id");
        int wechatDuration = intent.getIntExtra("wechat_duration", 60);
        
        // 如果是微信小程序命令，转换为统一格式
        if (wechatAction != null && !wechatAction.isEmpty()) {
            // 转换微信命令格式
            if ("wechat_record".equals(wechatAction)) {
                action = "record";
                duration = wechatDuration;
            } else if ("wechat_photo".equals(wechatAction)) {
                action = "photo";
            }
            // 使用微信命令ID作为会话ID
            conversationId = wechatCommandId;
            conversationType = "wechat";
            userId = "wechat_mini";
            
            // 清除微信命令，避免重复执行
            intent.removeExtra("wechat_action");
            intent.removeExtra("wechat_command_id");
            intent.removeExtra("wechat_duration");
            
            AppLog.d(TAG, "Received WeChat command: " + wechatAction + ", commandId: " + wechatCommandId);
        }
        ...
        // 创建 final 变量副本，供 lambda 使用
        final String finalAction = action;
        final String finalConversationId = conversationId;
        final String finalConversationType = conversationType;
        ...
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
            ...
            if (cameraManager == null) {
                ...
                executeRemoteCommand(finalAction, finalConversationId, finalConversationType, finalUserId, finalDuration);
                return;
            }
    ...
    private void setupLayoutByCarModel() {
        ...
        AppLog.d(TAG, "车型配置: carModel=" + carModel + ", cameraCount=" + cameraCount);
        
        // 领克07：单摄像头布局（摄像头编号0，分辨率2560x1600）
        if (AppConfig.CAR_MODEL_LYNKCO_07.equals(carModel)) {
            ...
            AppLog.d(TAG, "使用领克07配置：单摄像头布局（摄像头编号0，分辨率=" + appConfig.getTargetResolution() + "）");
        }
    ...
    private void initCamerasForLynkCo07(String[] cameraIds) {
        ...
        if (textureFront != null && frontId != null) {
            ...
            AppLog.d(TAG, "领克07初始化：摄像头ID=" + frontId + ", 分辨率=" + appConfig.getTargetResolution());
        } else {
            Toast.makeText(this, "没有可用的摄像头", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void uploadRecordedVideos() {
        ...
        // 判断是微信小程序还是钉钉
        final String currentWechatCmdId = wechatCommandId;
        final boolean isWechatCommand = "wechat".equals(remoteConversationType) || 
                (currentWechatCmdId != null && !currentWechatCmdId.isEmpty());
        
        if (isWechatCommand && wechatCloudManager != null) {
            // 微信小程序：上传到云存储
            AppLog.d(TAG, "微信小程序录像上传: " + filesToUpload.size() + " 个文件");
            handleWechatVideoResult(filesToUpload, currentWechatCmdId, uploadFromTempDir);
        } else if (dingTalkApiClient != null && remoteConversationId != null) {
            // 钉钉：使用原有上传逻辑
     ...
     * 上传视频到云存储，并报告结果
     */
    private void handleWechatVideoResult(List<File> videoFiles, String cmdId, boolean fromTempDir) {
        AppLog.d(TAG, "处理微信小程序录像结果: " + videoFiles.size() + " 个视频");
        
        runOnUiThread(() -> {
            Toast.makeText(MainActivity.this, "正在上传 " + videoFiles.size() + " 个视频...", Toast.LENGTH_SHORT).show();
        });
        
        // 上传视频到云存储
        uploadVideosToCloudAsync(videoFiles, cmdId, (successCount, failCount, fileIds) -> {
            // 构建结果信息
            StringBuilder resultInfo = new StringBuilder();
            resultInfo.append("录像完成，");
            if (failCount == 0) {
                resultInfo.append("成功上传 ").append(successCount).append(" 个视频");
            } else {
                resultInfo.append("上传 ").append(successCount).append(" 个成功，")
                          .append(failCount).append(" 个失败");
            }
            
            // 报告命令结果
            if (cmdId != null && wechatCloudManager != null) {
                wechatCloudManager.reportCommandResult(cmdId, failCount == 0, resultInfo.toString());
            }
            
            // 传输临时文件到最终目录
            if (fromTempDir) {
                transferTempFilesToFinalDir(videoFiles);
            }
            
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, resultInfo.toString(), Toast.LENGTH_SHORT).show();
                returnToBackgroundIfRemoteWakeUp();
            });
            
            AppLog.d(TAG, "微信小程序录像处理完成: " + resultInfo);
        });
    }
     ...
     * 批量上传视频到微信云存储
     */
    private void uploadVideosToCloudAsync(List<File> files, String commandId, 
            com.kooo.evcam.wechat.WechatCloudManager.BatchUploadCallback callback) {
        if (wechatCloudManager == null) {
            if (callback != null) {
                callback.onComplete(0, files.size(), new java.util.ArrayList<>());
            }
            return;
        }
        ...
        new Thread(() -> {
            ...
            for (File file : files) {
                try {
                    String deviceId = wechatMiniConfig.getDeviceId();
                    String fileName = file.getName();
                    long now = System.currentTimeMillis();
                    
                    String cloudPath = "videos/" + deviceId + "/" + fileName;
                    String fileId = wechatCloudManager.uploadFileToCloud(file, cloudPath);
                    ...
                    if (fileId != null) {
                        ...
                        // 记录视频信息到云数据库
                        wechatCloudManager.addFileRecord(fileId, deviceId, fileName, "video", 
                                file.length(), cloudPath, commandId);
                        
                        AppLog.d(TAG, "视频 " + successCount + "/" + files.size() + " 上传成功: " + fileName);
                    } else {
                        failCount++;
                    }
                } catch (Exception e) {
            ...
            if (callback != null) {
                runOnUiThread(() -> callback.onComplete(finalSuccess, finalFail, fileIds));
            }
        }).start();
    }
    ...
    private void uploadPhotos() {
        ...
        // 判断是微信小程序还是钉钉
        if ("wechat".equals(remoteConversationType)) {
            // 微信小程序：报告拍照成功，照片保存在本地
            handleWechatPhotoResult(recentFiles);
        } else if (dingTalkApiClient != null && remoteConversationId != null) {
            // 钉钉：上传照片
     ...
     * 上传照片到云存储，并报告结果
     */
    private void handleWechatPhotoResult(List<File> photoFiles) {
        AppLog.d(TAG, "处理微信小程序拍照结果: " + photoFiles.size() + " 张照片");
        
        final String cmdId = remoteConversationId;
        
        // 上传照片到云存储
        if (wechatCloudManager != null) {
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, "正在上传 " + photoFiles.size() + " 张照片...", Toast.LENGTH_SHORT).show();
            });
            
            wechatCloudManager.uploadPhotosToCloudAsync(photoFiles, cmdId, 
                    (successCount, failCount, fileIds) -> {
                // 构建结果信息
                StringBuilder resultInfo = new StringBuilder();
                resultInfo.append("拍照完成，");
                if (failCount == 0) {
                    resultInfo.append("成功上传 ").append(successCount).append(" 张照片");
                } else {
                    resultInfo.append("上传 ").append(successCount).append(" 张成功，")
                              .append(failCount).append(" 张失败");
                }
                ...
                AppLog.d(TAG, "微信小程序拍照处理完成: " + resultInfo);
            });
        } else {
            // 没有云服务，直接返回
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, "拍照完成: " + photoFiles.size() + " 张（未上传）", Toast.LENGTH_SHORT).show();
                returnToBackgroundIfRemoteWakeUp();
            });
        }
    }
    ...
    /**
     * 启动微信云开发服务（使用官方HTTP API）
     * 自动启动，不需要配置自建服务器
     */
    public void startWechatCloudService() {
        if (wechatCloudManager != null && wechatCloudManager.isRunning()) {
            AppLog.d(TAG, "微信云开发服务已在运行");
            return;
        }

        AppLog.d(TAG, "正在启动微信云开发服务...");
        ...
        WechatCloudManager.ConnectionCallback connectionCallback = new WechatCloudManager.ConnectionCallback() {
            ...
            public void onConnected() {
                runOnUiThread(() -> {
                    AppLog.d(TAG, "微信云开发服务已连接");
                    Toast.makeText(MainActivity.this, "微信云开发服务已连接", Toast.LENGTH_SHORT).show();
                    updateWechatMiniFragmentUI();
                });
            }
        ...
        // 创建并启动微信云开发管理器
        wechatCloudManager = new WechatCloudManager(this, wechatMiniConfig, connectionCallback);
        wechatCloudManager.start(commandCallback);
    }

    /**
     * 停止微信云开发服务
     */
    public void stopWechatCloudService() {
        if (wechatCloudManager != null) {
            AppLog.d(TAG, "正在停止微信云开发服务...");
            wechatCloudManager.stop();
            wechatCloudManager = null;
            Toast.makeText(this, "微信云开发服务已停止", Toast.LENGTH_SHORT).show();
            updateWechatMiniFragmentUI();
        }
    }
     ...
     * 获取微信云开发服务运行状态
     */
    public boolean isWechatCloudServiceRunning() {
    ...
    public void startPreviewStream() {
        ...
        if (wechatCloudManager == null || !wechatCloudManager.isRunning()) {
            AppLog.w(TAG, "微信云服务未运行，无法启动预览");
            return;
        }
        
        if (cameraManager == null || !cameraManager.hasConnectedCameras()) {
            AppLog.w(TAG, "没有可用的摄像头，无法启动预览");
            return;
        }
        
        isPreviewStreaming = true;
        previewHandler = new android.os.Handler(android.os.Looper.getMainLooper());
        ...
        AppLog.d(TAG, "实时预览流已启动，间隔: " + PREVIEW_INTERVAL + "ms");
    }
    ...
    public void stopPreviewStream() {
        ...
        AppLog.d(TAG, "实时预览流已停止");
    }
    
    /**
     * 获取预览流状态
     */
    public boolean isPreviewStreaming() {
        return isPreviewStreaming;
    }
    ...
    private void captureAndUploadPreviewFrame() {
        ...
        // 从第一个可用的摄像头获取预览帧
        cameraManager.capturePreviewFrame((jpegData, cameraPosition) -> {
            if (jpegData != null && wechatCloudManager != null) {
                wechatCloudManager.uploadPreviewFrame(jpegData, fileId -> {
                    AppLog.d(TAG, "预览帧上传成功: " + cameraPosition);
                });
            }
        });
    }
    ...
    public boolean isWechatMiniServiceRunning() {
        // 同时检查旧的 StreamManager 和新的 CloudManager
        return (wechatMiniStreamManager != null && wechatMiniStreamManager.isRunning())
                || (wechatCloudManager != null && wechatCloudManager.isRunning());
    }
    
    /**
     * 启动微信云管理器（简化版，用于新的绑定界面）
     * 复用已有的 startWechatCloudService 方法
     */
