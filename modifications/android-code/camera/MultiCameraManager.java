...
public class MultiCameraManager {
    ...
    // 全景摄像头模式相关
    private boolean panoramicMode = false;  // 是否是全景模式（单摄像头4方向裁切）
    private String panoramicCameraId = null;  // 全景摄像头ID
    private TextureView[] panoramicTextureViews = null;  // 全景模式的4个TextureView
    private String[] panoramicPositions = {"front", "back", "left", "right"};  // 全景方向映射
    ...
    /**
     * 初始化全景摄像头模式（单摄像头4方向裁切）
     * @param cameraId 全景摄像头ID
     * @param frontView 前方向TextureView
     * @param backView 后方向TextureView
     * @param leftView 左方向TextureView
     * @param rightView 右方向TextureView
     */
    public void initPanoramicCamera(String cameraId, TextureView frontView, 
                                     TextureView backView, TextureView leftView, 
                                     TextureView rightView) {
        AppLog.d(TAG, "初始化全景摄像头模式: cameraId=" + cameraId);
        
        this.panoramicMode = true;
        this.panoramicCameraId = cameraId;
        this.panoramicTextureViews = new TextureView[]{frontView, backView, leftView, rightView};
        
        // 清空之前的摄像头
        cameras.clear();
        
        if (cameraId != null && frontView != null) {
            // 创建单个摄像头实例，用于全景模式
            SingleCamera camera = new SingleCamera(context, cameraId, frontView);
            camera.setCameraPosition("front");
            camera.setPrimaryInstance(true);
            
            // 获取全景配置
            AppConfig appConfig = new AppConfig(context);
            String topLeftMapping = appConfig.getPanoramicMapping("top_left");
            String topRightMapping = appConfig.getPanoramicMapping("top_right");
            String bottomLeftMapping = appConfig.getPanoramicMapping("bottom_left");
            String bottomRightMapping = appConfig.getPanoramicMapping("bottom_right");
            
            // 设置全景配置
            camera.setPanoramicConfig(
                new String[]{topLeftMapping, topRightMapping, bottomLeftMapping, bottomRightMapping},
                new int[]{
            ...
            // 设置鱼眼校正
            boolean fisheyeEnabled = appConfig.isPanoramicFisheyeCorrectionEnabled();
            int fisheyeStrength = appConfig.getPanoramicFisheyeStrength();
            camera.setFisheyeCorrection(fisheyeEnabled, fisheyeStrength);
            
            // 设置主视图的裁切区域
            camera.setPanoramicCropRegion(AppConfig.getPanoramicCropRegionByRegion("top_left"));
            
            // 设置额外的全景视图
            camera.setAdditionalPanoramicViews(
                new TextureView[]{backView, leftView, rightView},
                new float[][]{
                    AppConfig.getPanoramicCropRegionByRegion("top_right"),
                    AppConfig.getPanoramicCropRegionByRegion("bottom_left"),
                    AppConfig.getPanoramicCropRegionByRegion("bottom_right")
                }
            );
            
            cameras.put("front", camera);
            AppLog.d(TAG, "初始化全景摄像头: ID=" + cameraId + ", 启用4方向裁切");
            AppLog.d(TAG, "  方向映射: 左上=" + topLeftMapping + ", 右上=" + topRightMapping + 
                ", 左下=" + bottomLeftMapping + ", 右下=" + bottomRightMapping);
            AppLog.d(TAG, "  鱼眼校正: " + (fisheyeEnabled ? "启用, 强度=" + fisheyeStrength : "禁用"));
        }
        
        AppLog.d(TAG, "全景摄像头模式初始化完成");
        
        // 设置回调
        setupPanoramicCameraCallbacks();
        
        // 清空录制器（全景模式录制器将在录制开始时创建）
        recorders.clear();
    ...
    public void updatePanoramicWheelOffset(float offset) {
        if (panoramicMode) {
            ...
            if (camera != null) {
                camera.updatePanoramicWheelOffset(offset);
            }
        }
    }
     ...
     * 设置全景摄像头回调
     */
    private void setupPanoramicCameraCallbacks() {
        CameraCallback callback = new CameraCallback() {
            @Override
            public void onCameraOpened(String cameraId) {
                AppLog.d(TAG, "Panoramic camera " + cameraId + " opened");
                if (statusCallback != null) {
                    statusCallback.onCameraStatusUpdate(cameraId, "已打开");
                }
            }
            ...
            @Override
            public void onCameraConfigured(String cameraId) {
                AppLog.d(TAG, "Panoramic camera " + cameraId + " configured");
                if (statusCallback != null) {
                    statusCallback.onCameraStatusUpdate(cameraId, "预览已启动");
                }
                ...
                if (camera != null) {
                    ...
                    // 启动全景帧更新（将主摄像头帧分割到其他TextureView）
                    camera.startPanoramicFrameUpdate();
                }
                ...
                synchronized (sessionLock) {
                    if (expectedSessionCount > 0) {
                        ...
                        sessionConfiguredCount++;
                        AppLog.d(TAG, "Panoramic session configured: " + sessionConfiguredCount + "/" + expectedSessionCount);
                        if (sessionConfiguredCount >= expectedSessionCount && pendingRecordingStart != null) {
                            AppLog.d(TAG, "Panoramic session ready, starting recording...");
                            if (sessionTimeoutRunnable != null) {
                                mainHandler.removeCallbacks(sessionTimeoutRunnable);
                                sessionTimeoutRunnable = null;
                            }
                            mainHandler.postDelayed(pendingRecordingStart, 300);
                        }
                    }
                }
            }
            ...
            @Override
            public void onCameraClosed(String cameraId) {
                AppLog.d(TAG, "Panoramic camera " + cameraId + " closed");
                if (statusCallback != null) {
                    statusCallback.onCameraStatusUpdate(cameraId, "已关闭");
                }
            }
            ...
            public void onCameraError(String cameraId, int errorCode) {
                String errorMsg = getErrorMessage(errorCode);
                AppLog.e(TAG, "Panoramic camera " + cameraId + " error: " + errorCode + " - " + errorMsg);
                if (statusCallback != null) {
                    statusCallback.onCameraStatusUpdate(cameraId, "错误: " + errorMsg);
                }
            }
            ...
            @Override
            public void onPreviewSizeChosen(String cameraId, Size previewSize) {
                AppLog.d(TAG, "Panoramic camera " + cameraId + " preview size: " + previewSize);
            }
        };
    ...
    public interface PreviewFrameCallback {
        void onFrameCaptured(byte[] jpegData, String cameraPosition);
    }
    ...
    public void capturePreviewFrame(PreviewFrameCallback callback) {
        ...
        for (Map.Entry<String, SingleCamera> entry : cameras.entrySet()) {
            ...
            if (camera.isConnected()) {
                ...
                camera.capturePreviewFrame((jpegData) -> {
                    if (jpegData != null) {
                        callback.onFrameCaptured(jpegData, position);
                    }
                });
                return;
            }
        ...
        AppLog.w(TAG, "No connected camera for preview frame capture");
    }
