...
public class SingleCamera {
    ...
    // ==================== 全景模式相关字段 ====================
    private float[] panoramicCropRegion = null;  // 全景裁切区域 [x, y, width, height]
    private boolean isPanoramicMode = false;  // 是否是全景模式
    private TextureView[] additionalTextureViews = null;  // 额外的TextureView（用于显示其他方向）
    private float[][] additionalCropRegions = null;  // 额外的裁切区域
    private Surface[] additionalSurfaces = null;  // 额外的Surface
    private int[] panoramicRotations = null;  // 各方向旋转角度
    private String[] panoramicDirections = null;  // 方向映射 [front, back, left, right]
    ...
    private float panoramicWheelXOffset = 0.0f;  // 全景滚轮X偏移
    private Paint drawPaint = null;  // 绘制画笔
    private Matrix[] cachedMatrices = null;  // 缓存的变换矩阵
    private RectF[] cachedSrcRects = null;  // 缓存的源矩形
    private RectF[] cachedDstRects = null;  // 缓存的目标矩形
    private int[] cachedRotations = null;  // 缓存的旋转角度
    private int[] cachedCropParams = null;  // 缓存的裁切参数
    private int cachedFullWidth = 0;  // 缓存的全帧宽度
    private int cachedFullHeight = 0;  // 缓存的全帧高度
    private Bitmap[] frameBuffers = new Bitmap[2];  // 帧缓冲（双缓冲）
    private int currentBufferIndex = 0;  // 当前缓冲索引
    private final Object bufferLock = new Object();  // 缓冲锁
    private Handler mainHandler = null;  // 主线程Handler（用于全景帧更新）
    private Handler panoramicFrameHandler = null;  // 全景帧处理Handler
    ...
    public void setPanoramicCropRegion(float[] cropRegion) {
        ...
        if (cropRegion != null) {
            AppLog.d(TAG, "Camera " + cameraId + " panoramic crop region set: [" + 
                cropRegion[0] + ", " + cropRegion[1] + ", " + cropRegion[2] + ", " + cropRegion[3] + "]");
        } else {
            AppLog.d(TAG, "Camera " + cameraId + " panoramic mode disabled");
        }
    }
    ...
    /**
     * 设置额外的全景视图
     * @param textureViews 额外的TextureView数组
     * @param cropRegions 对应的裁切区域数组
     */
    public void setAdditionalPanoramicViews(TextureView[] textureViews, float[][] cropRegions) {
        this.additionalTextureViews = textureViews;
        this.additionalCropRegions = cropRegions;
        if (textureViews != null) {
            AppLog.d(TAG, "Camera " + cameraId + " set " + textureViews.length + " additional panoramic views");
        }
    }
    
    /**
     * 设置全景配置
     * @param directions 方向映射数组 [top_left, top_right, bottom_left, bottom_right]
     * @param rotations 对应的旋转角度数组
     */
    ...
    /**
     * 更新全景滚轮X偏移
     * @param offset 偏移量 (-1.0 到 1.0)
     */
    public void updatePanoramicWheelOffset(float offset) {
        this.panoramicWheelXOffset = offset;
        AppLog.d(TAG, "Camera " + cameraId + " panoramic wheel X offset: " + offset);
    }
    ...
    public void startPanoramicFrameUpdate() {
        if (!isPanoramicMode || additionalTextureViews == null || 
            additionalCropRegions == null || panoramicFrameUpdateRunning) {
            return;
        }
        ...
        panoramicFrameThread = new HandlerThread("PanoramicFrame", android.os.Process.THREAD_PRIORITY_DISPLAY);
        ...
        panoramicFrameRunnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    processPanoramicFrameOptimized();
                } catch (Exception e) {
                ...
                panoramicFrameHandler.postDelayed(this, Math.max(1L, PANORAMIC_FRAME_INTERVAL_MS - elapsed));
            }
        };
        panoramicFrameHandler.postDelayed(panoramicFrameRunnable, 300L);
    ...
    private void initObjectPool() {
        int length = additionalTextureViews.length;
        if (drawPaint == null) {
            drawPaint = new Paint(Paint.FILTER_BITMAP_FLAG);
            drawPaint.setDither(false);
        }
        cachedRotations = new int[length];
        for (int i = 0; i < length; i++) {
            cachedRotations[i] = getRotationForRegion(i + 1);
        }
        cachedMatrices = new Matrix[length];
        cachedSrcRects = new RectF[length];
        cachedDstRects = new RectF[length];
        for (int i = 0; i < length; i++) {
            cachedMatrices[i] = new Matrix();
            cachedSrcRects[i] = new RectF();
            cachedDstRects[i] = new RectF();
        }
        cachedCropParams = new int[length * 4];
    }
     ...
     * 优化的全景帧处理
     */
    private void processPanoramicFrameOptimized() {
        if (textureView == null || !textureView.isAvailable()) {
            return;
        }
        ...
        final Bitmap bitmap;
        synchronized (bufferLock) {
            int idx = currentBufferIndex;
            Bitmap buffer = frameBuffers[idx];
            if (buffer == null || buffer.isRecycled()) {
                bitmap = textureView.getBitmap();
            } else {
                Bitmap result = textureView.getBitmap(buffer);
                bitmap = (result != null) ? result : textureView.getBitmap();
            }
            if (bitmap == null) {
                return;
            }
            frameBuffers[idx] = bitmap;
            currentBufferIndex = (idx + 1) % 2;
            ...
            if (panoramicWheelXOffset != 0.0f) {
                applyWheelOffsetToCropParams(width);
            }
            
            final Bitmap finalBitmap = bitmap;
            mainHandler.post(() -> drawToAdditionalViews(finalBitmap));
        }
    }
     ...
     * 绘制到额外的视图
     */
    private void drawToAdditionalViews(Bitmap bitmap) {
        if (!panoramicFrameUpdateRunning) {
            return;
        }
        for (int i = 0; i < additionalTextureViews.length; i++) {
            TextureView tv = additionalTextureViews[i];
            ...
            try {
                drawFrameToCanvas(canvas, bitmap, i, tv.getWidth(), tv.getHeight());
            } finally {
    ...
    private void updateCachedCropParams(int fullWidth, int fullHeight) {
        for (int i = 0; i < additionalTextureViews.length; i++) {
            ...
            if (region != null) {
                int idx = i * 4;
                cachedCropParams[idx] = (int) (region[0] * fullWidth);
                cachedCropParams[idx + 1] = (int) (region[1] * fullHeight);
                cachedCropParams[idx + 2] = (int) (region[2] * fullWidth);
                cachedCropParams[idx + 3] = (int) (region[3] * fullHeight);
            }
        }
    }
     ...
     * 应用滚轮偏移到裁切参数
     */
    private void applyWheelOffsetToCropParams(int fullWidth) {
        for (int i = 1; i < additionalTextureViews.length; i++) {
            int idx = i * 4;
            int cropWidth = cachedCropParams[idx + 2];
            int baseX = (int) (additionalCropRegions[i][0] * fullWidth);
            int offsetX = (int) (cropWidth * panoramicWheelXOffset);
            cachedCropParams[idx] = Math.max(0, Math.min(baseX + offsetX, fullWidth - cropWidth));
        }
    }
     ...
     * 绘制帧到画布
     */
    private void drawFrameToCanvas(Canvas canvas, Bitmap bitmap, int viewIndex, int viewWidth, int viewHeight) {
        int idx = viewIndex * 4;
        int cropX = cachedCropParams[idx];
        int cropY = cachedCropParams[idx + 1];
        int cropW = cachedCropParams[idx + 2];
        int cropH = cachedCropParams[idx + 3];
        
        RectF srcRect = cachedSrcRects[viewIndex];
        RectF dstRect = cachedDstRects[viewIndex];
        Matrix matrix = cachedMatrices[viewIndex];
        ...
        int rotation = cachedRotations[viewIndex];
        if (rotation != 0) {
     ...
     * 获取区域对应的旋转角度
     */
    private int getRotationForRegion(int regionIndex) {
        if (panoramicDirections != null && panoramicRotations != null) {
            if (regionIndex >= 0 && regionIndex < 4) {
                String direction = (regionIndex < panoramicDirections.length) ? 
                    panoramicDirections[regionIndex] : null;
                if (direction == null) {
                    return 0;
                }
                ...
                if (dirIdx >= 0 && dirIdx < panoramicRotations.length) {
                    return panoramicRotations[dirIdx];
                }
            }
        }
        return 0;
     ...
     * 返回 JPEG 格式的图片数据
     */
    public void capturePreviewFrame(PreviewFrameCallback callback) {
        if (callback == null) return;
        
        if (textureView == null || !textureView.isAvailable()) {
            AppLog.w(TAG, "Camera " + cameraId + " TextureView not available for preview capture");
        ...
        if (backgroundHandler != null) {
            backgroundHandler.post(() -> {
                try {
                    ...
                    callback.onFrameCaptured(jpegData);
                    
                } catch (Exception e) {
                    AppLog.e(TAG, "Failed to capture preview frame", e);
                    callback.onFrameCaptured(null);
                }
            });
        } else {
